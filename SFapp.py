...
with tab1:
    if data is not None:
        try:
            ...
            stack = stackk(bb, selected_Sdate, selected_Edate, sdate, edate, selected_variables, prcSum, prcNrd, Di,
                           tmaxMax, tminMin, tmeanMean, zinc, srtm)
            training = get_training(bb, vectors, selected_variables, prcSum, prcNrd, Di, tmaxMax, tminMin, tmeanMean,
                                    zinc, srtm)
            ...
            st.error(
                "An error occurred: Try again. Ensure the AOI geometry is a polygon or rectangle. Very large or very small regions might not be processed either")
        ...
        try:
            ...
            st.sidebar.markdown(
                "Are you satisfied with the results? If yes, Click 'Get Results' to download the results for use.")
            outDiss = download_data(res, stack, bb, selected_variables, prcSum, prcNrd, Di, tmaxMax, tminMin, tmeanMean,
                                    zinc, srtm)
