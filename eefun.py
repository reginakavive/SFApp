...
def subset(feature, selected_variables):
    ...
    selected_properties_dict = {var: feature.get(properties_mapping[var]) for var in selected_variables if
                                var in properties_mapping}
...
def prcSum(selected_Sdate, selected_Edate, sdate, edate, bb):
    ...
    prc = ee.ImageCollection('UCSB-CHG/CHIRPS/DAILY') \
        ...
        .filter(
        ee.Filter.calendarRange(int(selected_Sdate.strftime("%-m")), int((selected_Edate).strftime("%-m")), 'month')) \
...
def stackk(bb, selected_Sdate, selected_Edate, sdate, edate, selected_variables, prcSum, prcNrd, Di, tmaxMax, tminMin,
    ...
    stack = ee.ImageCollection.fromImages([
        prcSum.unitScale(ee.Number(rescale(moving_var(prcSum).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
                         rescale(prcSum.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(
            crs='EPSG:4326', scale=1000).toFloat().rename('prcSum'),
        prcNrd.unitScale(ee.Number(rescale(moving_var(prcNrd).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
                         rescale(prcNrd.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(
            crs='EPSG:4326', scale=1000).toFloat().rename('prcNrd'),
        Di.unitScale(ee.Number(rescale(moving_var(Di).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
                     rescale(Di.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(crs='EPSG:4326',
                                                                                                   scale=1000).toFloat().rename(
            'Di'),
        tmaxMax.unitScale(
            ee.Number(rescale(moving_var(tmaxMax).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
            rescale(tmaxMax.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(crs='EPSG:4326',
                                                                                               scale=1000).toFloat().rename(
            'tmaxMax'),
        tminMin.unitScale(
            ee.Number(rescale(moving_var(tminMin).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
            rescale(tminMin.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(crs='EPSG:4326',
                                                                                               scale=1000).toFloat().rename(
            'tminMin'),
        tmeanMean.unitScale(
            ee.Number(rescale(moving_var(tmeanMean).reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
            rescale(tmeanMean.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(crs='EPSG:4326',
                                                                                                 scale=1000).toFloat().rename(
            'tmeanMean'),
        zinc.unitScale(ee.Number(rescale(zinc.reproject(crs='EPSG:4326', scale=1000)).values().get(1)),
                       rescale(zinc.reproject(crs='EPSG:4326', scale=1000)).values().get(0)).reproject(crs='EPSG:4326',
                                                                                                       scale=1000).toFloat().rename(
            'zinc'),
...
def get_numClusters(bb, training, selected_variables, x):
    ...
    return (distinct_clusters_count)
...
def download_data(res, stack, bb, selected_variables, prcSum, prcNrd, Di, tmaxMax, tminMin, tmeanMean, zinc, srtm,slp,SOCmean,pHmean,CECmean,Nmean,claymean,sandmean):
    ...
    def propfn(prop, selected_properties):
        ...
        f = ee.Feature(tempFC.geometry(), {
            ...
            **{prop_name: ee.Number(prop_value).format('%.3e') for prop_name, prop_value in
               aggregated_properties.items()}
        })
        return f
